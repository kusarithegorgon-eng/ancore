...
export function IfElseNode({ node }: { node: CanvasNode }) {
  ...
  return (
    ...
    >
      <div className="absolute -left-1.5 top-1/2 -translate-y-1/2 h-3 w-3 rounded-full bg-[#121214] border-2 border-[#B57BFF] z-20" style={{ boxShadow: "0 0 8px rgba(181,123,255,0.6)" }} />
      <div className="absolute -right-1.5 top-1/3 -translate-y-1/2 h-3 w-3 rounded-full bg-[#121214] border-2 border-[#B57BFF] z-20" style={{ boxShadow: "0 0 8px rgba(181,123,255,0.6)" }}>
        <span className="absolute -top-4 left-1/2 -translate-x-1/2 text-[8px] text-[#B57BFF] font-semibold whitespace-nowrap">True</span>
      </div>
      <div className="absolute left-1/2 -bottom-1.5 -translate-x-1/2 h-3 w-3 rounded-full bg-[#121214] border-2 border-[#B57BFF] z-20" style={{ boxShadow: "0 0 8px rgba(181,123,255,0.6)" }}>
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-3">
          <span className="inline-flex items-center gap-1.5 text-[9px] uppercase tracking-[0.18em] font-semibold px-2 py-0.5 rounded-full border border-[rgba(181,123,255,0.3)] text-[color:var(--purple)] bg-[rgba(181,123,255,0.08)]">Logic</span>
        </div>
        ...
        <div className="space-y-2 mb-3">
          <div>
            ...
            <input type="text" value={data.propertyKey ?? ""} onChange={(e) => store.updateNodeData(node.id, { propertyKey: e.target.value })} className="w-full h-7 px-2 rounded-md bg-[#0a0a0c] border border-border-subtle text-[12px] text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-[color:var(--purple)]/40" placeholder="data.status" onClick={(e) => e.stopPropagation()} />
          </div>
          <div className="flex gap-2">
            <div className="flex-1">
              <label className="block text-[10px] uppercase tracking-wider text-muted-foreground font-medium mb-1">Compare</label>
              <select value={data.comparison ?? "equals"} onChange={(e) => store.updateNodeData(node.id, { comparison: e.target.value as any })} className="w-full h-7 px-2 rounded-md bg-[#0a0a0c] border border-border-subtle text-[12px] text-foreground focus:outline-none focus:border-[color:var(--purple)]/40 appearance-none" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1">
              ...
              <input type="text" value={data.comparisonValue ?? ""} onChange={(e) => store.updateNodeData(node.id, { comparisonValue: e.target.value })} className="w-full h-7 px-2 rounded-md bg-[#0a0a0c] border border-border-subtle text-[12px] text-foreground placeholder:text-muted-foreground/70 focus:outline-none focus:border-[color:var(--purple)]/40" placeholder="active" onClick={(e) => e.stopPropagation()} />
            </div>
          </div>
        </div>
